...
const BASE_URL = 'https://mi-app-omaolvi2za-uc.a.run.app';

// ─── ESCENARIO PRINCIPAL ───────────────────────────────────────────
export default function () {
  // 70% tráfico normal
  const r1 = http.get(`${BASE_URL}/api/Product`);
  ...
  // 20% tráfico lento
  const r2 = http.get(`${BASE_URL}/api/Product/delay/400`);
  ...
  if (Math.random() < 0.1) {
    const r3 = http.get(`${BASE_URL}/api/Product/error500/100`);
